 ...
 *  の責任を負わない．
 * 
 *  $Id: serial.c 971 2014-01-12 14:02:10Z ertl-hiro $
 */
 ...
 */
ER
_serial_opn_por(ID portid, ID cdmid)
{
	SPCB	*p_spcb;
	ER		ercd;
 ...
 */
ER
_serial_cls_por(ID portid, ID cdmid)
{
	SPCB	*p_spcb;
	ER		ercd;
	bool_t	eflag = false;
...
serial_wri_chr(SPCB *p_spcb, char c)
{
	...
	if (c == '\n' && (p_spcb->ioctl & IOCTL_CRLF) != 0U) {
		...
		if ((bool_t) rercd) {
			/*
			 *  CRを送信した時点でバッファフルになった場合．
			 */
			SVC(rercd = wai_sem(p_spcb->p_spinib->snd_semid),
										gen_ercd_wait(rercd, p_spcb));
			/* 上のwai_semからE_RLWAI，E_DLTが返った時は，セマフォを
				獲得していないため，何もせずにリターンしてよい．*/
		}
	}
 ...
 */
ER_UINT
_serial_wri_dat(ID portid, const char *buf, uint_t len, ID cdmid)
{
	SPCB	*p_spcb;
	bool_t	buffer_full;
	uint_t	wricnt = 0U;
	...
	if (!PROBE_MEM_READ_SIZE(buf, len)) {
		return(E_MACV);				/* メモリアクセス権のチェック */
	}
	...
	while (wricnt < len) {
		...
		SVC(rercd = serial_wri_chr(p_spcb, *buf++),
										gen_ercd_wait(rercd, p_spcb));
		/* 上のserial_wri_chrからE_RLWAI，E_DLTが返った時は，CRを送信
		　	して，LFを送る前にバッファフルになった場合である．バッファ
			フルであり，セマフォも獲得していないため，何もせずにリター
			ンしてよい．*/
		wricnt++;
 ...
 */
ER_UINT
_serial_rea_dat(ID portid, char *buf, uint_t len, ID cdmid)
{
	SPCB	*p_spcb;
	bool_t	buffer_empty;
	uint_t	reacnt = 0U;
	...
	if (!PROBE_MEM_WRITE_SIZE(buf, len)) {
		return(E_MACV);				/* メモリアクセス権のチェック */
	}
	...
	while (reacnt < len) {
		...
		*buf++ = c;
		...
		if ((p_spcb->ioctl & IOCTL_ECHO) != 0U) {
			...
			/* 上のserial_wri_chrからE_RLWAI，E_DLTが返った時は，CRを送
				信して，LFを送る前にバッファフルになった場合である．バッ
				ファフルであり，セマフォも獲得していないため，何もせず
				にリターンしてよい．*/
			if (!((bool_t) rercd)) {
 ...
 */
ER
_serial_ctl_por(ID portid, uint_t ioctl, ID cdmid)
{
	SPCB	*p_spcb;
 ...
 */
ER
_serial_ref_por(ID portid, T_SERIAL_RPOR *pk_rpor, ID cdmid)
{
	SPCB	*p_spcb;
...
extsvc_serial_opn_por(intptr_t portid, intptr_t par2, intptr_t par3,
							intptr_t par4, intptr_t par5, ID cdmid)
{
	return((ER_UINT) _serial_opn_por((ID) portid, cdmid));
}
...
extsvc_serial_cls_por(intptr_t portid, intptr_t par2, intptr_t par3,
							intptr_t par4, intptr_t par5, ID cdmid)
{
	return((ER_UINT) _serial_cls_por((ID) portid, cdmid));
}
...
extsvc_serial_rea_dat(intptr_t portid, intptr_t buf, intptr_t len,
							intptr_t par4, intptr_t par5, ID cdmid)
{
	return((ER_UINT) _serial_rea_dat((ID) portid, (char *) buf,
													(uint_t) len, cdmid));
}
...
extsvc_serial_wri_dat(intptr_t portid, intptr_t buf, intptr_t len,
							intptr_t par4, intptr_t par5, ID cdmid)
{
	return((ER_UINT) _serial_wri_dat((ID) portid,
								(const char *) buf, (uint_t) len, cdmid));
}
...
extsvc_serial_ctl_por(intptr_t portid, intptr_t ioctl, intptr_t par3,
							intptr_t par4, intptr_t par5, ID cdmid)
{
	return((ER_UINT) _serial_ctl_por((ID) portid, (uint_t) ioctl, cdmid));
}
...
extsvc_serial_ref_por(intptr_t portid, intptr_t pk_rpor, intptr_t par3,
							intptr_t par4, intptr_t par5, ID cdmid)
{
	return((ER_UINT) _serial_ref_por((ID) portid,
									(T_SERIAL_RPOR *) pk_rpor, cdmid));
}
